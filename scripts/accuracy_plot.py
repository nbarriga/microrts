...
for alg in range(num): 
    res.append(lines[alg].translate(None, "[],").split())
    del res[alg][-1]
counts = lines[num].translate(None, "[],").split()
del counts[-1]

counts = np.asarray(map(float, counts))
for alg in range(num): 
...
for alg in range(num): 
    for i in range(20):
        i_sd[alg][i] = dev(res[alg][i], counts[i])*120.0/math.sqrt(counts[i])
    res[alg] = res[alg] * 100 / counts
